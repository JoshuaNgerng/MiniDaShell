...
int	get_token(t_tokens *t, char *input, int start)
{
	int	i;
...
t_tokens	*ft_split_tokens(char *input)
{
	...
	while (input[i])
	{
		...
		temp->next = new;
		if (!new)
			return (free_tokens(head), sp_errmsg(0, "t_tokens (malloc): "),  NULL);
		i = get_token(new, input, i);
		if (errno)
			return (free_tokens(head), sp_errmsg(0, "t_tokens (substr): "), NULL);
		temp = temp->next;
	}
	temp->next = NULL;
	return (head);
