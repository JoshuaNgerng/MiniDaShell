...
t_data	*malloc_data_node(int size[5])
{
	...
	if (!out->cmd)
		return (free(out), free(out->f_read), NULL);
	out->here_doc = (char **) malloc ((size[2] + 1) * sizeof(char *));
	if (!out->here_doc)
		return (free(out), free(out->f_read), free(out->here_doc), NULL);
	out->f_out = (t_file *) malloc ((size[3] + 1) * sizeof(t_file));
	if (!out->f_write)
		return (free(out), free(out->f_read), free(out->here_doc), \
				free(out->f_write), NULL);
	return (out);
}
